...
print_nodes :: proc(root: ^types.Node, indent: string = "  ", is_root_node: bool = true) {
    ...
    if node.parent == nil || is_root_node {
        inner_indent = strings.concatenate({ left_padding, "      " }, context.temp_allocator);

        fmt.printf("[INFO]:{}| Printing node...\n{}--- ", indent, left_padding);
        fmt.printf("[{1}] -> {{ \n{0}Parent: '{2}' [{3}] (%p), \n{0}id: {5}, \n{0}style: {6}," +
        "\n{0}children [{7}]: {{", inner_indent, node.tag, node.parent != nil ? node.parent.tag : "nil",
        node.parent != nil ? node.parent.id : 0, node.parent, node.id, node.style, len(node.children));
    } else {
        fmt.printf("\n{0}[{}] -> {{ \n{0}  Parent: '{}' [%d] (%p), \n{0}  id: {}, \n{0}  style: {}, " +
        "\n{0}  children [{}]: {{", indent, node.tag, node.parent.tag, node.parent.id,
        node.parent, node.id, node.style, len(node.children));
    }
